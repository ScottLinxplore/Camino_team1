    ...
    useEffect(() => {
        fetch('https://test-camino.onrender.com/data?table=img')
            ...
            .then(data => {
                ...
                const ImgData_1 = data.find(item => item.target_id === 9)
    ...
    useEffect(() => {
        fetch('https://test-camino.onrender.com/data?table=routes')
            ...
            .then((data) => {
                ...
                const intro = data.find(item => item.route_id === 2)
                setCityIntro(intro)
            })
    }, [])

    return (
        <div style={{  display: 'flex', gap: '150px',backgroundColor:'white',borderRadius:'10px',boxShadow:'0 4px 8px rgba(0,0,0,0.1)' }}>
            <div style={{ display: 'flex', flexDirection: 'column', marginLeft: '100px' }}>
                <h2 style={{ fontWeight: 'bold' }}>銀之路 <img style={{ width: '150px', marginTop: '30px' }} src={foot} alt="" /></h2>
