...
const Sight_des1 = () => {
  ...
  useEffect(() => {
    fetch('https://test-camino.onrender.com/data?table=img')
      ...
      .then(data => {
        ...
        const ImgData_1 = data.find(item => item.target_id === 13)
  ...
  useEffect(() => {
    fetch('https://test-camino.onrender.com/data?table=img')
      ...
      .then(data => {
        ...
        const ImgData_2 = data.find(item => item.target_id === 13)
  ...
  useEffect(() => {
    fetch('https://test-camino.onrender.com/data?table=sight')
      .then(res => res.json())
      .then(data => {
        const description = data.find(item => Number(item.sight_id) === 3)
    ...
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginTop: '30px', marginBottom: '10px' }}>
        <div style={{ marginRight: '70%', fontSize: '30px' }}>
          女王之橋 <br />
          <span style={{ fontSize: '20px' }}>Puente la Reina</span>
      ...
      {/* 本體-第一部分 */}
      <div style={{ marginBottom: '30px', alignItems: 'center', flexDirection: 'column', marginTop: '30px', display: 'flex', width: '1000px', justifyContent: 'center', marginLeft: 'auto', marginRight: 'auto', backgroundColor: 'white', borderRadius: '10px', boxShadow: '0 4px 8px rgba(0,0,0,0.1)' }} >
        {sight1 ? (<img
          src={sight1}
          alt="底部圖片"
          style={{
        ...
        <p style={{ width: '80%' }}>{description1 ? description1.description : "資料載入中"}</p>

      </div>

      {/* 本體-第二部分 */}
      <div style={{ marginBottom: '30px', alignItems: 'center', flexDirection: 'column', marginTop: '90px', display: 'flex', width: '1000px', justifyContent: 'center', marginLeft: 'auto', marginRight: 'auto', backgroundColor: 'white', borderRadius: '10px', boxShadow: '0 4px 8px rgba(0,0,0,0.1)' }} >
        {sight2 ? (<img
          src={sight2}
          alt="底部圖片"
          style={{
        ...
        <p style={{ width: '80%' }}>{description1 ? description1.description_2 : "資料載入中"}</p>
