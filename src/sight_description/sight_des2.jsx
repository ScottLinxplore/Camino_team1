...
const Sight_des1 = () => {
  ...
  useEffect(() => {
    fetch('https://test-camino.onrender.com/data?table=img')
      ...
      .then(data => {
        ...
        const ImgData_1 = data.find(item => item.target_id === 12)
  ...
  useEffect(() => {
    fetch('https://test-camino.onrender.com/data?table=img')
      ...
      .then(data => {
        ...
        const ImgData_2 = data.find(item => item.target_id === 12)
  ...
  useEffect(() => {
    fetch('https://test-camino.onrender.com/data?table=sight')
      .then(res => res.json())
      .then(data => {
        const description = data.find(item => Number(item.sight_id) === 2)
    ...
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginTop: '30px', marginBottom: '10px' }}>
        <div style={{ marginRight: '70%', fontSize: '30px' }}>
          潘普洛納 <br />
          <span style={{ fontSize: '20px' }}>Pamplona</span>
          {/* 愛心切換設置 */}
        </div>
        <div onClick={() => {
          SetIslike(!islike)
