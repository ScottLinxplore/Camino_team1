  ...
  return (
    <>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.tabButtons}>
          {["weather", "altitude", "map"].map((key) => (
            ...
            >
              {key === "weather" ? "天氣預報" : key === "altitude" ? "城市天氣" : "海拔圖示"}
            </button>
          ))}
        </div>
        ...
        <div className={styles.tabContent}>
          {activeTab === "weather" && <div className={styles.weatherBox}><AverageWeather /></div>}
          {activeTab === "altitude" && (
          ...
          {activeTab === "map" && <div className={styles.mapBox}><Altitude /></div>}
        </div>
        ...
        <div className={styles.bottomSection}>
          ...
          <div className={`${styles.card} ${styles.leftCard}`}>
            ...
            <p style={{padding:'15px'}}>{cityIntro ? cityIntro.intro : "資料載入中"}</p>
          </div>

          {/* 右邊卡片 */}
          <div className={`${styles.card} ${styles.rightCard}`}>
            <img src={temperory} alt="朝聖地圖" className={styles.tempImage} />
            <p className={styles.cardTitle}>【法國之路】朝聖之旅-路線規畫方針</p>
            <div className={styles.buttonRow}>
              <button className={styles.greenButton}>立刻訂購套裝行程</button>
              <button className={styles.greenButton}>規劃路線法國之路</button>
            </div>
          </div>
        </div>
      </div>
      ...
      {showDescription && (
        <div className={styles.descriptionText}>
          {cityIntro && cityIntro.description ? (
            <div>
              <span className={styles.descriptionFirstChar}>{cityIntro.description.charAt(0)}</span>
      ...
      <div className={styles.routeButtons}>
        <button onClick={() => navigate("/Location")} className={styles.routeButton}>景點介紹</button>
        <button onClick={() => navigate("/Albergue")} className={styles.routeButton}>庇護所資訊</button>
        <button onClick={() => navigate("/Stamp")} className={styles.routeButton}>蓋章地點</button>
      </div>
      ...
      <div className={styles.viewRoutesButton}>
        <button onClick={() => navigate("/HomeRoute")} className={styles.routeButton}>瀏覽所有路線</button>
      </div>
